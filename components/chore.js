...
function isAuthorized(request) {
  ...
  return (
    ...
    authHeader.split(" ")[1] === CONFIG.API_KEY
  );
}
...
async function handleChatCompletions(request) {
  try {
    ...
    const model =
      CONFIG.CUSTOMER_MODEL_MAP[requestedModel] ||
      CONFIG.CUSTOMER_MODEL_MAP["SD-XL-Lightning-CF"];
...
function determinePromptModel() {
  return CONFIG.USE_EXTERNAL_API &&
    CONFIG.EXTERNAL_API &&
    CONFIG.EXTERNAL_MODEL &&
    CONFIG.EXTERNAL_API_KEY
    ? CONFIG.EXTERNAL_MODEL
    : CONFIG.CF_TRANSLATE_MODEL;
}
...
async function postRequest(model, jsonBody) {
  const cf_account =
    CONFIG.CF_ACCOUNT_LIST[
      Math.floor(Math.random() * CONFIG.CF_ACCOUNT_LIST.length)
    ];
